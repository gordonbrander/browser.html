...
const WindowControls = Component(({webViewer}) => {
  const theme = webViewer.get('theme'),
        closeColor = theme.get('close'),
        minColor = theme.get('min'),
        maxColor = theme.get('max'),
        glyphsShowing = theme.get('glyphsShowing');

  return DOM.div({className: 'windowctrls' +
                  (glyphsShowing ? ' glyphsshowing' : '')}, [
...
const NavigationControls = Component(({input, webViewer, title}) => {
  const theme = webViewer.get('theme');
  const buttonColor = theme.get('buttons');
  const titleColor = theme.get('title');
  const domainColor = theme.get('domain');

  const location = webViewer.get('location')
  ...
  return DOM.div({
    ...
    DOM.p({key: 'page-info',
      ...
      DOM.span({key: 'location',
                ...
                location ? url.getDomainName(location) : ''),
      DOM.span({key: 'title',
                style: {color: titleColor},
                className: 'pagetitle'},
               title ? title :
...
const NavigationPanel = Component(({key, input, webViewer, title}) => {
  ...
  const backgroundColor = theme.get('background');
